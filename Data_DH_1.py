...
def data_read(s, r):
    Label = pd.read_csv('c:/Users/Hi Windows 11 Home/Documents/AI lab/SOTA/Datasets/Label.csv')
    ...
    for i in range(1, 7):
        vibr = pd.read_csv('c:/Users/Hi Windows 11 Home/Documents/AI lab/SOTA/Datasets/{}.csv'.format(vibr_list[i-1]), nrows=r).drop('Index', axis=1).reset_index(drop=True).values.reshape([-1, 4000])
        vibr1 = np.array(vibr)
